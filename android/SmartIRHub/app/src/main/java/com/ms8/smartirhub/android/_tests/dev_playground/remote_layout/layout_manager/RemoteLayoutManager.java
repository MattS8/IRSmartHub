 ...
 */
public class RemoteLayoutManager extends RecyclerView.LayoutManager implements
    ...
    @NonNull
    private final RemoteLayoutState mLayoutState;
     ...
     */
    @SuppressWarnings("unused")
    public RemoteLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
                               int defStyleRes) {
        Properties properties = getProperties(context, attrs, defStyleAttr, defStyleRes);
        setOrientation(properties.orientation);
        setSpanCount(properties.spanCount);
        setReverseLayout(properties.reverseLayout);
        mLayoutState = new RemoteLayoutState();
     ...
     */
    public RemoteLayoutManager(int spanCount, int orientation) {
    ...
    boolean checkForGaps() {
        ...
        int invalidGapDir = mShouldReverseLayout ? RemoteLayoutState.LAYOUT_START : RemoteLayoutState.LAYOUT_END;
...
//        width = chooseSize(wSpec, usedWidth, getMinimumWidth());
    ...
    private void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        if (anchorInfo.mLayoutFromEnd) {
            // Layout start.
            setLayoutStateDirection(RemoteLayoutState.LAYOUT_START);
            fill(recycler, mLayoutState, state);
            // Layout end.
            setLayoutStateDirection(RemoteLayoutState.LAYOUT_END);
        ...
        if (mPendingSavedState == null || mPendingSavedState.mAnchorPosition == RecyclerView.NO_POSITION
            ...
            if (child != null) {
                ...
                if (mPendingScrollPositionOffset == INVALID_OFFSET) {
                    ...
                    anchorInfo.mLayoutFromEnd = position == RemoteLayoutState.LAYOUT_END;
                    anchorInfo.assignCoordinateFromPadding();
                } else {
    ...
    private int computeScrollOffset(RecyclerView.State state) {
        ...
        return RemoteScrollbarHelper.computeScrollOffset(state, mPrimaryOrientation,
    ...
    private int computeScrollExtent(RecyclerView.State state) {
        ...
        return RemoteScrollbarHelper.computeScrollExtent(state, mPrimaryOrientation,
                findFirstVisibleItemClosestToStart(!mSmoothScrollbarEnabled),
                findFirstVisibleItemClosestToEnd(!mSmoothScrollbarEnabled),
    ...
    private int computeScrollRange(RecyclerView.State state) {
        ...
        return RemoteScrollbarHelper.computeScrollRange(state, mPrimaryOrientation,
                findFirstVisibleItemClosestToStart(!mSmoothScrollbarEnabled),
                findFirstVisibleItemClosestToEnd(!mSmoothScrollbarEnabled),
                this, mSmoothScrollbarEnabled);
    }
...
//        }
    ...
    private void setLayoutStateDirection(int direction) {
        ...
        mLayoutState.mItemDirection = (mShouldReverseLayout == (direction == RemoteLayoutState.LAYOUT_START))
                ? RemoteLayoutState.ITEM_DIRECTION_TAIL : RemoteLayoutState.ITEM_DIRECTION_HEAD;
    }
    ...
    public void onItemsRemoved(RecyclerView recyclerView, int positionStart, int itemCount) {
        handleUpdate(positionStart, itemCount, RemoteAdapterHelper.UpdateOp.REMOVE);
    }

    @Override
    public void onItemsAdded(RecyclerView recyclerView, int positionStart, int itemCount) {
        handleUpdate(positionStart, itemCount, RemoteAdapterHelper.UpdateOp.ADD);
    }
    ...
    public void onItemsMoved(RecyclerView recyclerView, int from, int to, int itemCount) {
        handleUpdate(from, to, RemoteAdapterHelper.UpdateOp.MOVE);
    }

    @Override
    public void onItemsUpdated(RecyclerView recyclerView, int positionStart, int itemCount,
                               Object payload) {
        handleUpdate(positionStart, itemCount, RemoteAdapterHelper.UpdateOp.UPDATE);
    }
    ...
    private void handleUpdate(int positionStart, int itemCountOrToPosition, int cmd) {
        ...
        if (cmd == RemoteAdapterHelper.UpdateOp.MOVE) {
    ...
    private int fill(RecyclerView.Recycler recycler, RemoteLayoutState layoutState,
                     RecyclerView.State state) {
        ...
        if (mLayoutState.mInfinite) {
            if (layoutState.mLayoutDirection == RemoteLayoutState.LAYOUT_END) {
        ...
        while (layoutState.hasMore(state)
            ...
            if (layoutState.mLayoutDirection == RemoteLayoutState.LAYOUT_END) {
                ...
                if (assignSpan && lp.mFullSpan) {
                    ...
                    fullSpanItem.mGapDir = RemoteLayoutState.LAYOUT_START;
                ...
                if (assignSpan && lp.mFullSpan) {
                    ...
                    fullSpanItem.mGapDir = RemoteLayoutState.LAYOUT_END;
                    fullSpanItem.mPosition = position;
                    mLazySpanLookup.addFullSpanItem(fullSpanItem);
                }
            }
            ...
            if (lp.mFullSpan && layoutState.mItemDirection == RemoteLayoutState.ITEM_DIRECTION_HEAD) {
                if (assignSpan) {
        ...
        final int diff;
        if (mLayoutState.mLayoutDirection == RemoteLayoutState.LAYOUT_START) {
    ...
    private void attachViewToSpans(View view, LayoutParams lp, RemoteLayoutState layoutState) {
        if (layoutState.mLayoutDirection == RemoteLayoutState.LAYOUT_END) {
            if (lp.mFullSpan) {
    ...
    private void recycle(RecyclerView.Recycler recycler, RemoteLayoutState layoutState) {
        if (!layoutState.mRecycle || layoutState.mInfinite) {
            return;
        }
        if (layoutState.mAvailable == 0) {
            // easy, recycle line is still valid
            if (layoutState.mLayoutDirection == RemoteLayoutState.LAYOUT_START) {
    ...
    private void updateRemainingSpans(Span span, int layoutDir, int targetLine) {
        ...
        if (layoutDir == RemoteLayoutState.LAYOUT_START) {
    ...
    private boolean preferLastSpan(int layoutDir) {
        if (mOrientation == HORIZONTAL) {
            return (layoutDir == RemoteLayoutState.LAYOUT_START) != mShouldReverseLayout;
        }
        return ((layoutDir == RemoteLayoutState.LAYOUT_START) == mShouldReverseLayout) == isLayoutRTL();
    }

    /**
     * Finds the span for the next view.
     */
    private Span getNextSpan(RemoteLayoutState layoutState) {
    ...
    private int calculateScrollDirectionForPosition(int position) {
        if (getChildCount() == 0) {
            return mShouldReverseLayout ? RemoteLayoutState.LAYOUT_END : RemoteLayoutState.LAYOUT_START;
        }
        final int firstChildPos = getFirstChildPosition();
        return position < firstChildPos != mShouldReverseLayout ? RemoteLayoutState.LAYOUT_START : RemoteLayoutState.LAYOUT_END;
    }
    ...
    public void collectAdjacentPrefetchPositions(int dx, int dy, RecyclerView.State state,
        ...
        for (int i = 0; i < mSpanCount; i++) {
            ...
            int distance = mLayoutState.mItemDirection == RemoteLayoutState.LAYOUT_START
    ...
    void prepareLayoutStateForDelta(int delta, RecyclerView.State state) {
        ...
        if (delta > 0) { // layout towards end
            layoutDir = RemoteLayoutState.LAYOUT_END;
            referenceChildPosition = getLastChildPosition();
        } else {
            layoutDir = RemoteLayoutState.LAYOUT_START;
    ...
    public View onFocusSearchFailed(View focused, int direction, RecyclerView.Recycler recycler,
        ...
        if (layoutDir == RemoteLayoutState.INVALID_LAYOUT) {
            return null;
        }
        ...
        final int referenceChildPosition;
        if (layoutDir == RemoteLayoutState.LAYOUT_END) { // layout towards end
            referenceChildPosition = getLastChildPosition();
        } else {
            referenceChildPosition = getFirstChildPosition();
        }
        ...
        boolean shouldSearchFromStart = !mReverseLayout == (layoutDir == RemoteLayoutState.LAYOUT_START);
     ...
     * @return {@link RemoteLayoutState#LAYOUT_START} or {@link RemoteLayoutState#LAYOUT_END} if focus direction
     * is applicable to current state, {@link RemoteLayoutState#INVALID_LAYOUT} otherwise.
     */
    private int convertFocusDirectionToLayoutDirection(int focusDirection) {
        switch (focusDirection) {
            case View.FOCUS_BACKWARD:
                if (mOrientation == VERTICAL) {
                    return RemoteLayoutState.LAYOUT_START;
                } else if (isLayoutRTL()) {
                    return RemoteLayoutState.LAYOUT_END;
                } else {
                    return RemoteLayoutState.LAYOUT_START;
                }
            ...
            case View.FOCUS_UP:
                return mOrientation == VERTICAL ? RemoteLayoutState.LAYOUT_START
                        : RemoteLayoutState.INVALID_LAYOUT;
            case View.FOCUS_DOWN:
                return mOrientation == VERTICAL ? RemoteLayoutState.LAYOUT_END
                        : RemoteLayoutState.INVALID_LAYOUT;
            case View.FOCUS_LEFT:
                return mOrientation == HORIZONTAL ? RemoteLayoutState.LAYOUT_START
                        : RemoteLayoutState.INVALID_LAYOUT;
            case View.FOCUS_RIGHT:
                return mOrientation == HORIZONTAL ? RemoteLayoutState.LAYOUT_END
                        : RemoteLayoutState.INVALID_LAYOUT;
            default:
                if (DEBUG) {
                    Log.d(TAG, "Unknown focus request:" + focusDirection);
                }
                return RemoteLayoutState.INVALID_LAYOUT;
        }
    ...
    class Span {
        ...
        void calculateCachedStart() {
            ...
            if (lp.mFullSpan) {
                ...
                if (fsi != null && fsi.mGapDir == RemoteLayoutState.LAYOUT_START) {
        ...
        void calculateCachedEnd() {
            ...
            if (lp.mFullSpan) {
                LazySpanLookup.FullSpanItem fsi = mLazySpanLookup
                        .getFullSpanItem(lp.getViewLayoutPosition());
                if (fsi != null && fsi.mGapDir == RemoteLayoutState.LAYOUT_END) {
