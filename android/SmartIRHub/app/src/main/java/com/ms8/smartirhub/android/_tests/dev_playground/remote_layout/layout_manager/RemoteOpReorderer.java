...
class RemoteOpReorderer {

    final Callback mCallback;

    RemoteOpReorderer(Callback callback) {
        mCallback = callback;
    }

    void reorderOps(List<RemoteAdapterHelper.UpdateOp> ops) {
    ...
    private void swapMoveOp(List<RemoteAdapterHelper.UpdateOp> list, int badMove, int next) {
        final RemoteAdapterHelper.UpdateOp moveOp = list.get(badMove);
        final RemoteAdapterHelper.UpdateOp nextOp = list.get(next);
        switch (nextOp.cmd) {
    ...
    void swapMoveRemove(List<RemoteAdapterHelper.UpdateOp> list, int movePos, RemoteAdapterHelper.UpdateOp moveOp,
                        int removePos, RemoteAdapterHelper.UpdateOp removeOp) {
        RemoteAdapterHelper.UpdateOp extraRm = null;
        ...
        if (moveOp.itemCount < removeOp.positionStart) {
            ...
            // move is removed.
            removeOp.itemCount--;
            moveOp.cmd = RemoteAdapterHelper.UpdateOp.REMOVE;
            moveOp.itemCount = 1;
        ...
        if (moveOp.positionStart <= removeOp.positionStart) {
            ...
            extraRm = mCallback.obtainUpdateOp(RemoteAdapterHelper.UpdateOp.REMOVE, moveOp.positionStart + 1, remaining, null);
    ...
    private void swapMoveAdd(List<RemoteAdapterHelper.UpdateOp> list, int move, RemoteAdapterHelper.UpdateOp moveOp, int add,
                             RemoteAdapterHelper.UpdateOp addOp) {
        int offset = 0;
        // going in reverse, first revert the effect of add
    ...
    void swapMoveUpdate(List<RemoteAdapterHelper.UpdateOp> list, int move, RemoteAdapterHelper.UpdateOp moveOp, int update,
                        RemoteAdapterHelper.UpdateOp updateOp) {
        RemoteAdapterHelper.UpdateOp extraUp1 = null;
        RemoteAdapterHelper.UpdateOp extraUp2 = null;
        // going in reverse, first revert the effect of add
        if (moveOp.itemCount < updateOp.positionStart) {
            ...
            updateOp.itemCount--;
            extraUp1 = mCallback.obtainUpdateOp(RemoteAdapterHelper.UpdateOp.UPDATE, moveOp.positionStart, 1, updateOp.payload);
        }
        // now affect of add is consumed. now apply effect of first remove
        if (moveOp.positionStart <= updateOp.positionStart) {
            ...
            extraUp2 = mCallback.obtainUpdateOp(
                    RemoteAdapterHelper.UpdateOp.UPDATE, moveOp.positionStart + 1, remaining,
                    updateOp.payload);
    ...
    private int getLastMoveOutOfOrder(List<RemoteAdapterHelper.UpdateOp> list) {
        boolean foundNonMove = false;
        for (int i = list.size() - 1; i >= 0; i--) {
            final RemoteAdapterHelper.UpdateOp op1 = list.get(i);
            if (op1.cmd == RemoteAdapterHelper.UpdateOp.MOVE) {
                if (foundNonMove) {
                    return i;
                }
            } else {
    ...
    interface Callback {
        ...
        RemoteAdapterHelper.UpdateOp obtainUpdateOp(int cmd, int startPosition, int itemCount, Object payload);

        void recycleUpdateOp(RemoteAdapterHelper.UpdateOp op);
    }
}
